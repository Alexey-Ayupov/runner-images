...
provisioner "powershell" {
  ...
  provisioner "powershell" {
    inline = [
      ...
      "while($true) { $imageState = Get-ItemProperty HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Setup\\State | Select ImageState; if($imageState.ImageState -ne 'IMAGE_STATE_GENERALIZE_RESEAL_TO_OOBE') { Write-Output $imageState.ImageState; Start-Sleep -s 10; Get-Content C:\\Windows\\System32\\Sysprep\\Panther\\setupact.log; Get-Content C:\\Windows\\System32\\Sysprep\\Panther\\setuperr.log  } else { break } }"
    ]
  }
